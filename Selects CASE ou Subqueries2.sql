...
FROM filmes
 WHERE id IN (
    ...
    FROM DVD 
	data_fabricaçao
    WHERE data_fabricaçao > '2020-01-01'
);
...
*/
SELECT num, 
	CONVERT(CHAR(10), data_fabricaçao, 103) AS data_fabricacao, 
	 DATEDIFF(MONTH, data_fabricaçao, GETDATE()) AS qtd_meses_desde_fabricacao
FROM DVD
WHERE filmes_id IN
(
	SELECT id
...
*/
SELECT ld.num AS num_dvd, ld.data_locacao, ld.data_devolucao,
       DATEDIFF(day, ld.data_locacao, ld.data_devolucao) AS dias_alugado,
       ld.valor AS valor_locacao
FROM locaçao ld
JOIN cliente c ON ld.clientenum_cadastro = c.num_cadastro
...
*/
SELECT c.nome, CONCAT(c.logradouro, ' ', c.num) AS endereco_completo,
       LEFT(c.cep, 5) + '-' + RIGHT(c.cep, 3) AS cep_formatado
FROM cliente c
JOIN locaçao l ON c.num_cadastro = l.clientenum_cadastro
WHERE l.DVD_Num = 10002;
